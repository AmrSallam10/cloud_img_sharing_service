...
pub async fn send(data: Vec<u8>, socket: Arc<UdpSocket>, address: &str) {
    ...
    while curr_block < block_num {
        ...
        for frag_id in frag_st..frag_end {
            ...
            socket.send_to(&frag, address).await.expect("Failed to send!");
        }
            ...
            Ok((bytes_read, src_addr)) => {
                ...
                // if we received a fragment before update the message with the incoming fragment
                    ...
                    if big_msg.received_len == big_msg.msg_len || (big_msg.received_len as usize / FRAG_SIZE) % BLOCK_SIZE == 0 {
                        ...
                        socket.send_to(&ack, src_addr.to_string()).await.expect("Failed to send!");
                    }
