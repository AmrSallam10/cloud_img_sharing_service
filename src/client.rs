...
use directory_of_service::ClientDirOfService;
mod fragment;
use commons::{Image, Msg, Type};
use commons::{ELECTION_PORT, SERVERS_FILEPATH, SERVICE_PORT};

const REQ_ID_LOG_FILEPATH: &str = "./req_id_log.txt";

fn get_servers(filepath: &str, mode: &str) -> Vec<(SocketAddr, SocketAddr)> {
    let contents =
...
async fn send_init_request_to_cloud(
    ...
    for server in &servers {
        ...
        socket
            .send_to(serialized_msg.as_bytes(), target_addr)
            .await
            .unwrap();
    }
...
async fn main() {
    ...
    let dir_of_service = ClientDirOfService::new();
    ...
    for pic_path in &pic_paths {
        ...
        // trigger election
        if let Some(chosen_server) =
            send_init_request_to_cloud(socket.clone(), mode, Type::ClientRequest(id)).await
        {
