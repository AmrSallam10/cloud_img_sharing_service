...
impl ClientBackend {
    pub async fn new(args: Vec<String>) -> Self {
        ...
        let id = get_req_id_log(REQ_ID_LOG_FILEPATH);
        let socket: Arc<UdpSocket> = Arc::new(UdpSocket::bind(ip).await.expect("Failed to bind to ip"));
    ...
    fn get_pic_paths(&self, filepath: &str) -> Vec<String> {
        let contents =
            std_fs::read_to_string(filepath).expect("Should have been able to read the file");
    ...
    async fn send_init_request_to_cloud(&self) -> Option<SocketAddr> {
        ...
        for server in &servers {
            ...
            let msg = Msg {
                ...
                receiver: server_ip,
                msg_type: Type::ClientRequest(self.req_id),
                payload: None,
            };
